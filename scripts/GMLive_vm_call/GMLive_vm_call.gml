...
function vm_group_call_on_construct(l_th,l_act,l_scope,l_stack){
	...
	if(is_numeric(l_v1)){
		...
		if(method_get_index(l_v1)==vm_v2_gml_thread_group_func_literal_ctr_impl){
			with (new vm_v2_GmlStructBase()) {
				gml_std_gml_internal_ArrayImpl_shift(l_args1);
				var l_args2=l_args1;
				var l_meta=method_get_self(l_v1);
				var l_self1=self;
				var l_result;
				while(true){
					...
					var l_th1=l_pg.h_call_v(l_meta[$"live:function"],l_args2,false);
					if(l_th1==undefined){
						l_result=undefined;
						break;
					}
