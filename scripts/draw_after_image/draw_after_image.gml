...
/// @param silhouette: whether to make the after image a solid shape
function draw_after_image(argument0, argument1, argument2, argument3, argument4) {
	...
	if !variable_instance_exists(id, "trail_x")
	{
		...
		spacing = argument1
		//take care with fade length if your length is slow and you have only 4 images the image that is 4th will not fade out before it is erased
	...
	if(surface_exists(am_surface)){
		surface_set_target(am_surface);
	    gpu_set_blendmode(bm_subtract);
	    draw_set_color(c_black);
	    draw_set_alpha(0.5);
	    draw_rectangle(0, 0, camW, camH, 0);
	    gpu_set_blendmode(bm_normal);
		
		camera_apply(view_camera[0]);
		
		if(silhouette) gpu_set_fog(1, colour, 0, 1);
		for(var i=1; i<number_of_after_images; i++)
		{
			...
			if trail_x[i] != x || trail_y[i] != y
			{
			    ...
			    if trail_x[i] != undefined
			    {
				    ...
				    draw_sprite_ext(sprite_index, image_index, trail_x[i], trail_y[i], image_xscale, image_yscale, image_angle, c_white, alpha[i])
			    }
			}
		}
		if(silhouette) gpu_set_fog(0, c_white, 0, 0);
